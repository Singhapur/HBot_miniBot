...
class ArduinoBridge(Node):
    def __init__(self):
        ...
        # Variable para saber si queremos ir en línea recta pura
    ...
    def cmd_vel_callback(self, msg):
        ...
        # NUEVO: ¿Queremos ir en línea recta? (Velocidad V alta, giro W casi cero)
    ...
    def procesar_paquete(self, id_paquete, data):
        if id_paquete == ID_ENCODERS and len(data) == 4:
            ...
            # --- NUEVO: ASISTENTE DE LÍNEA RECTA ---
            if self.modo_linea_recta:
                # Comparamos los ticks de las ruedas traseras
                # Si error > 0, la izquierda va más rápido. Si error < 0, la derecha va más rápido.
                error = float(ti) - float(td)
                
                # Kp (Ganancia): Cuánto de agresiva es la corrección. 
                # Empieza con 2.0. Si el robot zigzaguea mucho, bájalo a 1.0 o 0.5.
                kp = 2.5 
                correccion = int(error * kp)
                
                # Aplicamos tu idea: restamos al que va rápido, sumamos al que va lento
                self.pwm_izq -= correccion
                self.pwm_der += correccion
                
                # Nos aseguramos de no salirnos de los límites físicos (95 a 255)
                # NOTA: Usamos abs() y mantenemos la dirección que ya calculó cmd_vel
                self.pwm_izq = max(95, min(255, self.pwm_izq))
                self.pwm_der = max(95, min(255, self.pwm_der))
