...
class ControllerNode(Node):
    def __init__(self):
        ...
        # 1. SUBSCRIBER TO KEYBOARD
        self.sub_cmd_vel = self.create_subscription(Twist, '/cmd_vel', self.cmd_vel_callback, 10)
        
        # 2. SUBSCRIBER TO ENCODERS (For the PID)
        ...
        # IMU
        self.imu_yaw_rate = 0.0  # Guardará la velocidad de giro real de la IMU
    ...
    def imu_callback(self, msg):
        # angular_velocity.z nos dice a cuántos rad/s está girando el robot sobre sí mismo
    ...
    def cmd_vel_callback(self, msg):
        ...
        if self.straight_line_mode:      
            ...
            # For make turns we need more power, for this reason the PWM is set to 130
            pwm_l = 130
            pwm_r = 130
    ...
    def joint_callback(self, msg):
        # Receive ticks and apply PID if going in a straight line
        if self.straight_line_mode:
            rl_rads = abs(msg.velocity[2])
            rr_rads = abs(msg.velocity[3])
            
            # Encoder error
            error_encoders = float(rl_rads) - float(rr_rads)
            
            # IMU error, in negative, If it is negative (turning right), the error will be positive to correct to the left
            error_imu = -self.imu_yaw_rate
            
            imu_weight = 0.85       # 85%
            encoders_weight = 0.15  # 15% 
            # error with weight
            error = (error_encoders * encoders_weight) + ((error_imu * 10.0) * imu_weight)
            
            termino_i = 0.0
            
            # Save the correct error
            if self.dir_left == 1 and self.dir_right == 1:
                self.error_fwd += error
                self.error_fwd = max(-50.0, min(50.0, self.error_fwd))
                termino_i = self.error_fwd
                
            elif self.dir_left == 0 and self.dir_right == 0:
                self.error_bwd += error
                self.error_bwd = max(-50.0, min(50.0, self.error_bwd))
                termino_i = self.error_bwd

            # Correction is now: Present (P) + Past (I)
            correction = int((error * self.kp) + (termino_i * self.ki))
            
            # Apply correction to the base
    ...
    def publish_pwm(self):
        time_last_command = (self.get_clock().now() - self.last_cmd_time).nanoseconds / 1e9
        
        if time_last_command > 0.2:
            self.pwm_actual_left = 0
            self.pwm_actual_right = 0
            self.dir_left = 2  # Release
            self.dir_right = 2 # Release
            self.straight_line_mode = False
            self.error_fwd = 0.0
            self.error_bwd = 0.0
        # Convert absolute to signed values (positive/negative)
